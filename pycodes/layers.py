...
ColwiseMultLayer = Lambda(lambda l: l[0])
...
#         super().__init__(**kwargs)
#     def build(self, input_shape):
#         self.theta = self.add_weight(shape=(1, input_shape[1]), initializer='zeros', trainable=True, name='theta')
#         self.theta_exp = tf.clip_by_value(K.exp(self.theta), 1e-3, 1e-4)
#         super().build(input_shape)
#     def call(self, x):
#         return tf.identity(x)
#     
...
#             "Input and output dims are not compatible"
#         
#         self.kernel = self.add_weight(shape=(self.units,), initializer=self.kernel_initializer, name='kernel', regularizer = self.kernel_regularizer, constraint=self.kernel_constraint)
#         if self.user_bias:
