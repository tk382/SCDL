...
def train(adata, 
    ...
    #network is an object in ZINBConstDispAutoencoder
    
    model = network.model
    loss = network.loss
    # loss = ZINB(, model.get_layer('dispersion').theta_exp,nonmissing_indicator = nonmissing_indicator,debug = network.debug).loss
...
def NBevaluate(adata, network, optimizer='rmsprop',
        batch_size = 32,
        nonmissing_indicator=None):
...
def ZINBevaluate(adata, network, optimizer='rmsprop',
        batch_size = 32,
        nonmissing_indicator=None,
        ridge_lambda = 0.0):
    ...
    loss = ZINB(pi = model.get_layer('pi').pi, 
      theta = model.get_layer('dispersion').theta_exp,
...
def newtrain(adata, 
    ...
    loss = ZINB(pi, model.get_layer('dispersion').theta_exp,
                nonmissing_indicator = nonmissing_indicator,
                debug = network.debug).loss
